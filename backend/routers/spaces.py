...
@router.get("/list", response_model=list[CreateSpaceRequest])
def list_spaces(created_by: int,db: db_dependency):
    spaces = db.query(Spaces).filter(Spaces.created_by == created_by)
    return spaces.all()

@router.get("/{space_id}", response_model=CreateSpaceRequest)
...
@router.put("/{space_id}", response_model=CreateSpaceRequest)
def update_space(space_id: int, request: CreateSpaceRequest, db: db_dependency):
    space = db.query(Spaces).filter(Spaces.id == space_id).first()
    if not space:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
